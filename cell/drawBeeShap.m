...
%	drawBeeShap(cool(61),1,3,5,data,0);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%	*^1:data_ = [1,0,2,5;2,0,5,5;5,0,0,1;1,0,2,2];
%获知使用随机函数产生data
%	*^1:data = int32(rand(20,20) * 60);
%绘制蜂窝形状 drawBeeShap({'red','blue','green','yellow'},1,6,5,data,0);
%绘制三角形	drawBeeShap({'red','blue','green','yellow'},1,3,5,data,0);
%使用hot或cool来产生颜色
%	hot		*^2:drawBeeShap(hot(61),1,3,5,data,0);
%	cool	*^2:drawBeeShap(cool(61),1,3,5,data,0);
%	varargin : (扩展功能)
%	0	使用常量（使用于多次使用该函数时）
%		当常量是 0 时将返回常量，当常量是结构体是将使用该对象作为常量
%		*^2:c = drawBeeShap(cool(61),1,3,5,data,0,0,0);
%		*^2:drawBeeShap(cool(61),1,3,5,data,0,0,c);
%使用自动颜色
%	1	*^2:drawBeeShap(cool(61),1,3,5,data,0,1);
%		*^2:drawBeeShap(0,1,3,5,data,0,1);
%	2	*^2:drawBeeShap(cool(61),1,3,5,data,0,2,cool);
%		*^2:drawBeeShap(0,1,3,5,data,0,2,cool);
%	3	*^2:drawBeeShap(cool(61),1,3,5,data,0,3,@cool,61);
%		*^2:drawBeeShap(0,1,3,5,data,0,3,@cool,61);
%▲注意：可以在 varargin{1} == 0 其后补充颜色参数
%		*^2:drawBeeShap(cool(200),3,5,data,0,0,0,2,@cool)
%2017年01月18日09:29:53新增边框颜色
%例子
%	data = int32(rand(20,20) * 60);
%	drawBeeShap(cool(61),1,6,5,data,0);
%	>>>(等同于)    drawBeeShap(cool(61),'black',6,5,data,0);
%	>>>(等同于)    drawBeeShap(cool(61),[0 0 0],6,5,data,0);
%	drawBeeShap(cool(61),2,6,5,data,0);
%	drawBeeShap(cool(61),[0.5 0.5 0.5],6,5,data,0);
function c = drawBeeShap(colors_,borderColor,edges_,len_,data,isShow,varargin)
	...
	if (length(varargin) > 0)
		...
		if (varargin{1} == 0)
			...
			if (strcmp(class(varargin{2}),'struct') == 1)
				...
				c = constValue_();
				drawBeeShap(colors_,borderColor,edges_,len_,data,isShow,0,c,varargin(:,3:length(varargin)));
				return ;
			end
		else
			c = constValue_();
	...
	if (length(cdata) <= length(colors_))
		...
		if (edges_ == 6)
			forSix_(c,borderColor,len_,colorSheet,data,isShow);
		elseif (edges_ == 3)
			forThree(c,borderColor,len_,colorSheet,data,isShow);
		elseif (edges_ == 4)
			forFour(c,borderColor,len_,colorSheet,data,isShow);
		end
	else
		display(['颜色值不足']);
	end
...
function drawOneShap_(x_,y_,edges_,len_,color_,borderColor,varargin)
    ...
    if (length(varargin) == 2)
        text(x_,y_,num2str(varargin{2}));
    end
end

function forSix_(c,borderColor,len_,colorSheet,data,isShow)
	%colorSheet 颜色表
	...
	for i = 1 : w_%length(data)
        if isShow == 1
            drawOneShap_(x(i),y(i),6,len_,colorSheet(num2str(data(1,i))),borderColor,0,data(1,i));
        else
	...
	for i = 2 : h_
		for j = 1 : w_
			y(j) = y(j) + dY;
            if isShow == 1
                drawOneShap_(x(j),y(j),6,len_,colorSheet(num2str(data(1,i))),borderColor,0,data(i,j));
            else
                drawOneShap_(x(j),y(j),6,len_,colorSheet(num2str(data(1,i))),borderColor);
            end
		end
	end
end

function forThree(c,borderColor,len_,colorSheet,data,isShow) 
	%colorSheet 颜色表
	...
	for i = 1 : w_
        if isShow == 1
            drawOneShap_(x(i),y(i),3,r_,colorSheet(num2str(data(1,i))),borderColor,extDeg(mod(i,2) + 1),data(1,i));
        else
	...
	for i = 2 : h_
		...
		for j = 1 : w_
			...
			y(j) = y(j) + dY(ex2);
            if isShow == 1
                drawOneShap_(x(j),y(j),3,r_,colorSheet(num2str(data(i,j))),borderColor,extDeg(ex2),data(i,j));
            else
...
function forFour(c,borderColor,len_,colorSheet,data,isShow) 
	%colorSheet 颜色表
	...
	for i = 1 : w_
        if isShow == 1
            drawOneShap_(x(i),y,4,len__,colorSheet(num2str(data(1,i))),borderColor,c.pi_d4,data(1,i));
        else
	...
	for i = 2 : h_
		...
		for j = 1 : w_
            if isShow == 1
                drawOneShap_(x(j),y,4,len__,colorSheet(num2str(data(i,j))),borderColor,c.pi_d4,data(i,j));
            else
...
function forFour_(c,borderColor,len_,colorSheet,data,isShow) 
	%colorSheet 颜色表
	%找出第一个点 (x,y) -> (len_ / 2,len_ / 2)
	[h_,w_] = size(data);
	x = [len_ / 2];
	...
	for i = 1 : w_
        if isShow == 1
            drawOneShap_(x(i),y,4,len_,colorSheet(num2str(data(1,i))),borderColor,0,data(1,i));
        else
	...
	for i = 2 : h_
		...
		for j = 1 : w_
            if isShow == 1
                drawOneShap_(x(j),y,4,len_,colorSheet(num2str(data(i,j))),borderColor,0,data(i,j));
            else
